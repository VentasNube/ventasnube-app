...

{
    ...
    public function get_m_OLD($group_id = false)
    {
        // $group_id = 2;
        $db = \Config\Database::connect();
    ...
    public function get_user_workspace_permission($user_id = false, $ws_id = false)
    {
     ...
     ///  TRAE TODOS LOS MODULOS QUE TIENE USER PERMISION EL USUARIO(Se usa para la navegacion lateral)
     public function get_ws($ws_id = false)
     {
         helper('date');
        ...
        // $time = Time::parse( $now, $ws_zona_h);
         ...
         $plans_db->select('*');
         $plans_db->where('workspace_id', $ws_id);
    ...
    public function get_m($user_id = false, $ws_id = false, $ws_plan_id = false)
    {
    ...
    public function get_m_OKOLd($user_id = false, $ws_id = false, $ws_plan_id = false)
    {
          ...
          // UNo los espacios de trabajos y sus planes id para filtrar los modulos que devuelve
        //  $module_db->join('workspace', 'module_plan.plan_id = workspace.auth_permissions_id');
          ...
          if ($query) {
              foreach ($query as $row) {
                ...
                // Poner un filtro que solo imprima los modulos que estan dentro del plan que abona
                  if ($row['module_type_id'] == 0) {
                      $row_m[] = array(
                          'm_id' => $row['m_id'],
                          'm_type_action' => $row['m_type_action'],
    ...
    public function get_m_t($user_id = false, $ws_id = false)
    {
