...
async function getTemplateAjax(path, callback) {
    var source, template;
    //  await $.ajax({
    try {
        await $.ajax({
...
async function NO(path, callback) {
    var source, template;
    //  await $.ajax({
    try {
        await $.ajax({
...
async function fetch_template(path, callback) {
...
async function compile_template(withTemplate, inElement, withData, callback) {
    try {
...
// ARMA EL TEMPLATE PARA COMPILAR 2021 Async
async function compile_templateNO(withTemplate, inElement, withData, callback) {
    try {
        // alert('withTemplate: ' + withTemplate);
        //  alert('inElement: ' + inElement);
        //  alert('withData: ' + withData);
        //  alert('callback: ' + callback);
        let response = await fetch_template(withTemplate, withData);
        let targetDiv = document.getElementById(inElement);

        targetDiv.after(response);

        //########## PRUEBA

        /*await fetch_template(withTemplate, function(template) {
             // var targetDiv = (typeof inElement == 'string') ? $(inElement) : inElement;
             // targetDiv.html(template(withData));
             var targetDiv = document.getElementById(inElement);
...
// ARMA EL TEMPLATE PARA COMPILAR 2021 Async
async function renderHandlebarsTemplate(withTemplate, inElement, withData, callback) {
    try {
        await getTemplateAjax(withTemplate, function(template) {
            var targetDiv = (typeof inElement == 'string') ? $(inElement) : inElement;
            targetDiv.html(template(withData));
