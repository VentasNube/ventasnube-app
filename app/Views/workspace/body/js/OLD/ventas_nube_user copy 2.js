...
var session = new PouchDB('http://localhost:5984');
...
// TRAIGO LOS DATOS DEL CART Y IMPRIMO
async function getCart(user_db) {
    //   alert(user_db);
    // url_db = 'http://localhost:5984/userdb-736d6172746d6f62696c652e636f6d2e617240676d61696c2e636f6d';
    var user_db = new PouchDB(user_db);
    return await user_db.allDocs({
        include_docs: true,
        attachments: true,
        //startkey: 'foo',
        //  endkey: 'foo\ufff0'
        ...
        */
        //console.log('A VERRRRRRRR CARTTT ' + JSON.stringify(response.rows));
    });
}


async function getCartItems(user_db) {
    try {
        let doc = await getCart(user_db);
        alert(user_db);
        // console.log('GETTT CARRRRRRRRT' + doc);
    } catch (err) {
        console.log(err);
    }
}
...
function load_product_cart(user_db) {
    var user_db = new PouchDB(user_db);
    user_db.allDocs({ include_docs: true, descending: true }, function(err, doc) {
