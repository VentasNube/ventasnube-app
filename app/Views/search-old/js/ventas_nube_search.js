...
async function search_db() {
    ...
    try {
        ...
        //  R_search_db = new PouchDB(search_db);
        //  L_search_db = new PouchDB('http://localhost:5984/' + search_db, { skip_setup: true });
        //  R_search_db.sync(L_search_db, { live: true, retry: true, }); //sincronizo

        if (offline_mode) {
            L_search_db = new PouchDB(search_db);
            R_search_db = new PouchDB('http://localhost:5984/' + search_db, { skip_setup: false });
            ...
            L_search_db = new PouchDB('http://localhost:5984/' + search_db, { skip_setup: true });
            Snackbar.show({
                text: 'Catalogo OFFLINE Desactivado!',
                actionText: 'ok',
                actionTextColor: "#0575e6",
                pos: 'top-left'
            });
        ...
        //Variable global con la lista de precios
...
function variations_get(element) {
    ...
    let url_template = 'search/search_card_product_variant_template'; //NOMBRE CONTROLADOR TEMPLATE      
...
function variations_select(element) {
    ...
    let url_template = 'search/search_card_product_var_select_template'; //NOMBRE CONTROLADOR TEMPLATE      
