    ...
    try {
        let response = await dbUser.getSession(function(err, response) {
            if (err) {
                ...
                // alert('USER SESSION' + userInfo.userDb);
            }
        });
...
async function get_user(user_id) {
    ...
    return await dbUser.getUser(user_id, function(err, response) {
        if (err) {
            ...
            alert(response);
            response; // Devulevo el array resultado
            // response is the user object
        }
    });

    //dbUser.remove(item_cart_id, item_cart_rev); // post a new doc
    // return await db.get(response.id); // find by id
}
